...
//LRESULT CALLBACK TransparentProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
//{
//	switch(msg)
//	{
//		case WM_LBUTTONUP:
		...
		case WM_MOVING:
		{
			...
			if(lastTime != 0)
			{
				...
				if(deltaTime > 0)
				{
					...
					horizon_velocity = (double)dx / deltaTime * 16; // 标准化为16ms帧速
					velocity = (double)dy / deltaTime * 16; // 标准化为16ms帧速
				}
			}
