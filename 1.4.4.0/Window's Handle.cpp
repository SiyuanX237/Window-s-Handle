...
LRESULT CALLBACK ButtonPosProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	...
	switch(msg)
	{
		...
		case WM_RBUTTONUP:
		{
			if(!IsWindow(target))
			{
				KillTimer(g_MainHwnd, ID_Timer_Pos);
				ReleaseCapture();
				break;//非法窗口不干事，但需确保没有存在的Capture和定时器
			}
			KillTimer(g_MainHwnd, ID_Timer_Pos);
			...
			wsprintf(buf, L"%ld", rect.left);
			SetWindowText(g_EditPosX, buf);
			wsprintf(buf, L"%ld", rect.top);
...
LRESULT CALLBACK AboutProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(msg)
	{
		...
		case WM_PAINT:
		{
			...
			DrawText(hdc, TEXT("版本：1.4.4.0                日期：2026.3.12"), -1, &RectDrawText, DT_LEFT | DT_SINGLELINE);
