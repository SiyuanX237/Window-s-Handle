...
{
	//防止修改不能修改的窗口比如per-pixel型(这里是恢复过程)
	EnableWindow(g_EditTransparent, true);
	EnableWindow(g_ButtonTransparent, true);
	...
	{
		BYTE trans;
		if(!GetLayeredWindowAttributes(target, 0, &trans, NULL))
		{
			//防止修改不能修改的窗口比如per-pixel型
			EnableWindow(g_EditTransparent, false);
			EnableWindow(g_ButtonTransparent, false);
		}
...
void ReadWriteConfig()
{
	...
	if(!PathFileExists(g_ConfigPath))
	{
		...
		g_isTray = false;
		WritePrivateProfileString(L"Setting", L"ShowMainwindowHotkey", L"0", g_ConfigPath);
		g_HotkeyShowMainwindow = 0;
		WritePrivateProfileString(L"Setting", L"SelectHotkey", L"0", g_ConfigPath);
		g_HotkeySelect = 0;
		WritePrivateProfileString(L"Setting", L"SelectParentHotkey", L"0", g_ConfigPath);
		g_HotkeySelectParent = 0;
		WritePrivateProfileString(L"Setting", L"TransparentHotkey", L"0", g_ConfigPath);
		g_HotkeyTransparent = 0;
		WritePrivateProfileString(L"Setting", L"TopmostHotkey", L"0", g_ConfigPath);
		g_HotkeyTopmost = 0;
		WritePrivateProfileString(L"Setting", L"MoveHotkey", L"0", g_ConfigPath);
		g_HotkeyMove = 0;
		WritePrivateProfileString(L"Setting", L"SizeHotkey", L"0", g_ConfigPath);
		g_HotkeySize = 0;
	}
	else
	{
		...
		//获取快捷键信息
		g_HotkeyShowMainwindow = GetPrivateProfileInt(L"Setting", L"ShowMainwindowHotkey", 0, g_ConfigPath);
		g_HotkeySelect = GetPrivateProfileInt(L"Setting", L"SelectHotkey", 0, g_ConfigPath);
		g_HotkeySelectParent = GetPrivateProfileInt(L"Setting", L"SelectParentHotkey", 0, g_ConfigPath);
		g_HotkeyTransparent = GetPrivateProfileInt(L"Setting", L"TransparentHotkey", 0, g_ConfigPath);
		g_HotkeyTopmost = GetPrivateProfileInt(L"Setting", L"TopmostHotkey", 0, g_ConfigPath);
		g_HotkeyMove = GetPrivateProfileInt(L"Setting", L"MoveHotkey", 0, g_ConfigPath);
		g_HotkeySize = GetPrivateProfileInt(L"Setting", L"SizeHotkey", 0, g_ConfigPath);
	}
}
...
void LoadRules()
{
	...
	for(auto &x : result)
	{
		...
		{
			v.SizeOption = iTemp;//从INT16转INT8是合法的，iTemp来自ini的SizeOption字段，应该不会超过INT8
		}
		if(v.Flags & WH_SIZE)
		{
		...
		if(v.Flags & WH_ENABLE)
		{
			...
			{
				v.EnableRule = iTemp == 0 ? false : true;
			}
		}
...
void SaveRules(V *v,UINT16 index)
{
	...
	if(v->Flags & WH_ENABLE)
	{
		WritePrivateProfileString(cIndex, L"Enable", v->EnableRule ? L"1" : L"0", g_ConfigPath);
	}
...
bool ModPair(UINT16 key)
{
	...
	UINT or = (key >> 3) | key;
	UINT and = (key >> 3) & key;
	if(or &MOD_CONTROL)//有ctrl键
	{
		if(and &MOD_CONTROL)//都有
		{
			if(!(GetAsyncKeyState(VK_CONTROL) & 0x8000))//没有按下哦
			{
				return false;
			}
		}
		else if((key >> 3) & MOD_CONTROL)//只有左边
		{
			if(!(GetAsyncKeyState(VK_LCONTROL) & 0x8000))//没有按下哦
			{
				return false;
			}
		}
		...
		{
			if(!(GetAsyncKeyState(VK_RCONTROL) & 0x8000))//没有按下哦
			{
				return false;
			}
		}
	...
	if(or &MOD_SHIFT)//有shift键
	{
		if(and &MOD_SHIFT)//都有
		{
			if(!(GetAsyncKeyState(VK_SHIFT) & 0x8000))//没有按下哦
			{
				return false;
			}
		}
		else if((key >> 3) & MOD_SHIFT)//只有左边
		{
			if(!(GetAsyncKeyState(VK_LSHIFT) & 0x8000))//没有按下哦
			{
				return false;
			}
		}
		...
		{
			if(!(GetAsyncKeyState(VK_RSHIFT) & 0x8000))//没有按下哦
			{
				return false;
			}
		}
	...
	if(or &MOD_ALT)//有alt键
	{
		if(and &MOD_ALT)//都有
		{
			if(!(GetAsyncKeyState(VK_MENU) & 0x8000))//没有按下哦
			{
				return false;
			}
		}
		else if((key >> 3) & MOD_ALT)//只有左边
		{
			if(!(GetAsyncKeyState(VK_LMENU) & 0x8000))//没有按下哦
			{
				return false;
			}
		}
		...
		{
			if(!(GetAsyncKeyState(VK_RMENU) & 0x8000))//没有按下哦
			{
				return false;
			}
		}
