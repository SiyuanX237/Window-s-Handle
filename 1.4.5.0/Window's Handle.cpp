...
POINT g_LastPoint;//鼠标原点
RECT g_LastWindowRect;//窗口原矩形
HWND g_TargetWindow;//目标窗口句柄
HWND g_ShareTopTooltip = NULL;//共享窗口置顶时的气泡提示框句柄

//HSV转RGB
inline void HSVtoRGB(float h, float s, float v, RGB8 &out)
{
	float c = v * s;
...
//编辑框手动过滤字符，为了支持输入负数
auto EditNegaProc = [](HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)->LRESULT
{
	static HWND Tooltip = NULL;
	if (msg != WM_CHAR)return DefSubclassProc(hwnd, msg, wParam, lParam);
	...
	if(wParam == '-')
	{
		...
		char buf[20];
		GetWindowTextA(hwnd, buf, ARRAYSIZE(buf));//获取当前编辑框内的文本

		if(start == end)//插入模式
		{
			if(start != 0 || buf[0] == '-')//不是开头一定不合法，是开头也不能是'-'开头
			{
				RECT rect;
				GetWindowRect(hwnd, &rect);
				ShowTooltip(hwnd, &Tooltip, { rect.left,rect.bottom }, L"您只能在此处键入合法的整数", L"不能接受的字符", true, false, TTI_ERROR, 3000);
				return 0;//阻止输入
			}
		}
		else//块替换模式
	...
	else if(wParam >= '0' &&  wParam <= '9')//数字看看会不会插在负号前面
	{
		DWORD start, end;
		SendMessage(hwnd, EM_GETSEL, (WPARAM)&start, (LPARAM)&end);
		char buf[20];
...
//检测鼠标事件，低级钩子，尽可能快return
LRESULT CALLBACK MouseProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	if(nCode != HC_ACTION)return CallNextHookEx(g_MouseHook, nCode, wParam, lParam);;//只要普通消息
	switch(wParam)
	{
		case WM_MOUSEWHEEL:
		{
			...
			{
				if(!GetLayeredWindowAttributes(target, 0, &trans, NULL))return 1;//防止修改不能修改的窗口比如per-pixel型
			}
			else
			{
				trans = 255;
		...
		case WM_LBUTTONDOWN:
		{
			...
			GetCursorPos(&g_LastPoint);
			g_TargetWindow = GetAncestor(WindowFromPoint(g_LastPoint), GA_ROOT);
			if(!IsWindow(g_TargetWindow))break;
			GetWindowRect(g_TargetWindow, &g_LastWindowRect);
			g_HookStatus = 1;//正在move窗口
			return 1;//拦截左键按下
		}
		case WM_MOUSEMOVE:
		{
			if(!g_HookStatus) break;//0，非hook模式
			...
			else if(g_HookStatus == 2)//resize模式
			{
				LONG width = g_LastWindowRect.right - g_LastWindowRect.left;
				LONG height = g_LastWindowRect.bottom - g_LastWindowRect.top;

				LONG x1 = g_LastWindowRect.left + width / 3;
				LONG x2 = g_LastWindowRect.left + width * 2 / 3;
				LONG y1 = g_LastWindowRect.top + height / 3;
				LONG y2 = g_LastWindowRect.top + height * 2 / 3;

				int region = ((g_LastPoint.x < x1) ? 0 : (g_LastPoint.x < x2 ? 1 : 2)) + 3 * ((g_LastPoint.y < y1) ? 0 : (g_LastPoint.y < y2 ? 1 : 2));
				...
				switch(region)
				{
					...
					case 8:
					{
						SetWindowPos(g_TargetWindow, NULL, 0, 0, width + dx, height + dy, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOZORDER);
						break;
					}
					//左下角
					case 6:
					{
						SetWindowPos(g_TargetWindow, NULL, g_LastWindowRect.left + dx, g_LastWindowRect.top, width - dx, height + dy, SWP_NOACTIVATE | SWP_NOZORDER);
						break;
					}
					//左上角
					case 0:
					{
						SetWindowPos(g_TargetWindow, NULL, g_LastWindowRect.left + dx, g_LastWindowRect.top + dy, width - dx, height - dy, SWP_NOACTIVATE | SWP_NOZORDER);
						break;
					}
					//右上角
					case 2:
					{
						SetWindowPos(g_TargetWindow, NULL, g_LastWindowRect.left, g_LastWindowRect.top + dy, width + dx, height - dy, SWP_NOACTIVATE | SWP_NOZORDER);
						break;
					}
					//右边
					case 5:
					{
						SetWindowPos(g_TargetWindow, NULL, 0, 0, width + dx, height, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOZORDER);
						break;
					}
					//下边
					case 7:
					{
						SetWindowPos(g_TargetWindow, NULL, 0, 0, width, height + dy, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOZORDER);
						break;
					}
					//左边
					case 3:
					{
						SetWindowPos(g_TargetWindow, NULL, g_LastWindowRect.left + dx, g_LastWindowRect.top, width - dx, height, SWP_NOACTIVATE | SWP_NOZORDER);
						break;
					}
					//上边
					case 1:
					{
						SetWindowPos(g_TargetWindow, NULL, g_LastWindowRect.left, g_LastWindowRect.top + dy, width, height - dy, SWP_NOACTIVATE | SWP_NOZORDER);
						break;
					}
					//中间
					case 4:
					{
						SetWindowPos(g_TargetWindow, NULL, g_LastWindowRect.left - (dx >> 1), g_LastWindowRect.top - (dy >> 1), width + dx, height + dy, SWP_NOACTIVATE | SWP_NOZORDER);
						break;
					}
				}
			}
...
void CALLBACK HandleWinEvent(HWINEVENTHOOK hWinEventHook, DWORD event, HWND hwnd, LONG idObject, LONG idChild, DWORD dwEventThread, DWORD dwmsEventTime)
{
		...
		[](LPVOID Param)->DWORD
	{
		...
		for(const auto &x : Class啥[buf])
		{
			...
			{
				EnableWindow(hwnd, x->EnableRule);
			}
		...
		case WM_LBUTTONDOWN:
		{
			...
			if(!IsWindow(g_FrameHWND))//没有就创建
			{
				g_FrameHWND = CreateWindowEx(
					...
					NULL,
					WS_VISIBLE | WS_POPUP,
					0, 0, g_ScreenSize.cx, g_ScreenSize.cy - 1,
					NULL,
					NULL, NULL, NULL
				);
			}
...
LRESULT CALLBACK StyleProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(msg)
	{
		case WM_CREATE:
		{
			...
			wchar_t styletext[12];//编辑框字符串，临时变量
			//处理普通样式
			...
			//开始创建按钮s
			int i = g_FontSize.cy * 2.5;//控制y轴的间距
			for(auto &x : StyleFlags)
			{
				...
				//把当前按钮表示的样式值放到GWLP_USERDATA，方便事件驱动时取值，不要忘了！
				SetWindowLongPtr(temp, GWLP_USERDATA, (DWORD)x.flag);

				i += g_FontSize.cy * 1.2;
			}

			i = g_FontSize.cy * 2.5;//控制间距归位
		...
		case WM_COMMAND:
		{
			if(HIWORD(wParam) != BN_CLICKED)break;//仅处理点击事件
			if(LOWORD(wParam) == 114)//普通样式按钮
			{
				bool selected = SendMessage((HWND)lParam, BM_GETCHECK, 0, 0) == BST_CHECKED;
				//从当前按钮的GWLP_USERDATA取出表示的样式值
				if(selected)
				{
				...
				char styletext[12];//编辑框字符串，临时变量
				sprintf(styletext, "0x%08x", StyleValue);
				SetWindowTextA(EditStyle, styletext);
			}
			else if(LOWORD(wParam) == 514)//扩展样式按钮
			{
				bool selected = SendMessage((HWND)lParam, BM_GETCHECK, 0, 0) == BST_CHECKED;
				//从当前按钮的GWLP_USERDATA取出表示的样式值
				if(selected)
				...
				char styletext[12];//编辑框字符串，临时变量
				sprintf(styletext, "0x%08x", ExStyleValue);
				SetWindowTextA(EditExStyle, styletext);
			}
			else if(LOWORD(wParam) == 1919)//应用按钮
			{
		...
		case WM_PAINT:
		{
			...
			DrawText(hdc, TEXT("普通样式"), -1, &RectDrawText, DT_LEFT | DT_SINGLELINE);

			RectDrawText.left = 15 * g_FontSize.cx;
			RectDrawText.right = 20 * g_FontSize.cx;
			DrawText(hdc, TEXT("扩展样式"), -1, &RectDrawText, DT_LEFT | DT_SINGLELINE);
...
LRESULT CALLBACK EnumProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(msg)
	{
		case WM_CREATE:
		{
			...
			SortListAndUpdateArrow(ListHwnd, SortIndex, 0);
			break;
		}
		case WM_NOTIFY:
		{
			NMHDR *phdr = (NMHDR*)lParam;
			if(phdr->hwndFrom != ListHwnd)break;//确定消息是来自ListView控件的

			if(phdr->code == LVN_COLUMNCLICK)//单击某列，进行排序
			{
				NMLISTVIEW *pnmv = (NMLISTVIEW *)lParam;
				SortListAndUpdateArrow(ListHwnd, SortIndex, pnmv->iSubItem);//按照被点的那一列排序
			}
			else if(phdr->code == LVN_ITEMCHANGING)//处理选中项改变事件
			{
				NMLISTVIEW *p = (NMLISTVIEW *)lParam;
				if(!(p->uChanged & LVIF_STATE))break;//只处理状态变化事件

				if((p->uNewState & LVIS_SELECTED) == 0)break;//只有新项被选中才处理
				
				if((p->uOldState & LVIS_SELECTED) != 0)//旧项也被选中，说明用户想取消选中
				{
					//手动清除选中与焦点
					ListView_SetItemState(ListHwnd, p->iItem, 0, LVIS_SELECTED | LVIS_FOCUSED);
					//清除框框
					DestroyWindow(g_FrameHWND);
					g_FrameHWND = NULL;
					return true;// 阻止系统默认“再次选中”
				}
				else//旧项没有选中了，用户想选一个新项
				{
					//新建框框、改变框框位置过程
					if(!IsWindow(g_FrameHWND))//没有框框就新建，否则直接设置位置
					{
						g_FrameHWND = CreateWindowEx(
							WS_EX_TOOLWINDOW | WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_LAYERED | WS_EX_NOACTIVATE,
							L"Window's Handle - Frame",
					...
					if(iRow >= 0)
					{
						...
						PostMessage(g_FrameHWND, WM_Frame, 0, (LPARAM)(HWND)_wtoi(buf));
					}
				}
			}
			else if(phdr->code == NM_DBLCLK)//双击某项，选中并退出
			{
		...
		case WM_CONTEXTMENU:
		{
			...
			if((HWND)wParam == ListHwnd)
			{
				...
				//判断是否为表头
				RECT rcHeader;
				GetWindowRect(ListView_GetHeader(ListHwnd), &rcHeader);
				...
				hit.pt = pt;
				int RightClickIndex = ListView_HitTest(ListHwnd, &hit);
				//排除点到空白区域的情况
				if(RightClickIndex != -1 && (hit.flags & LVHT_ONITEM))
				{
					ListView_SetItemState(ListHwnd, RightClickIndex, LVIS_SELECTED, LVIS_SELECTED);
					GetCursorPos(&pt);
					HMENU hContextMenu = CreatePopupMenu();
					AppendMenu(hContextMenu, MF_STRING, ID_MENU_ChainParent, L"父级窗口链");
					AppendMenu(hContextMenu, MF_STRING, ID_MENU_ChainOwner, L"拥有窗口链");
		...
		case WM_COMMAND:
		{
			...
			{
				case ID_MENU_ChainParent:
				{
					RECT rect;
					GetWindowRect(hwnd, &rect);
					rect.left = ((rect.right + rect.left) >> 1) - 24 * g_FontSize.cx;
					rect.top = ((rect.bottom + rect.top) >> 1) - 15 * g_FontSize.cy;
					rect.right = rect.left + 48 * g_FontSize.cx;
					rect.bottom = rect.top + 30 * g_FontSize.cy;
					...
					wchar_t buf[HANDLESIZE];
					ListView_GetItemText(ListHwnd, ListView_GetNextItem(ListHwnd, -1, LVNI_FOCUSED), 0, buf, ARRAYSIZE(buf));//获取句柄
					ChainData *cd = (ChainData *)malloc(sizeof(ChainData));
					cd->type = true;
					cd->Target = (HWND)_wtoi(buf);
					g_ChainHWND = CreateWindowEx(
						(g_Noactivate == BST_CHECKED ? WS_EX_NOACTIVATE : NULL) | WS_EX_TOPMOST,
						L"Window's Handle - Chain",
						(wstring(buf)+wstring(L" - 父级窗口链")).c_str(),
				...
				case ID_MENU_ChainOwner:
				{
					...
					wchar_t buf[HANDLESIZE];
					ListView_GetItemText(ListHwnd, ListView_GetNextItem(ListHwnd, -1, LVNI_FOCUSED), 0, buf, ARRAYSIZE(buf));
					ChainData *cd = (ChainData *)malloc(sizeof(ChainData));
					cd->type = false;
					cd->Target = (HWND)_wtoi(buf);
					g_ChainHWND = CreateWindowEx(
						(g_Noactivate == BST_CHECKED ? WS_EX_NOACTIVATE : NULL) | WS_EX_TOPMOST,
						L"Window's Handle - Chain",
						(wstring(buf) + wstring(L" - 拥有窗口链")).c_str(),
						WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_POPUP | WS_SYSMENU,
						rect.left,
						rect.top,
						rect.right - rect.left,
...
//窗口归属窗口过程
LRESULT CALLBACK SetFollowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	static HWND EditTarget;
	static HWND EditNewParent;
	static HWND ButtonApply;
	static HWND ButtonSetType1;
	static HWND ButtonSetType2;
	char handle[HANDLESIZE];
	switch(msg)
	{
		case WM_CREATE:
		{
			ButtonSetType1 = CreateWindow(
				L"button",
				L"父系窗口",
			...
			SendMessage(ButtonSetType1, WM_SETFONT, (WPARAM)g_EditFont, 0);
			SendMessage(ButtonSetType1, BM_SETCHECK, BST_CHECKED, 0);

			ButtonSetType2 = CreateWindow(
				L"button",
				L"拥有窗口",
			...
			SendMessage(ButtonSetType2, WM_SETFONT, (WPARAM)g_EditFont, 0);
		...
		case WM_PAINT:
		{
			...
			RectDrawText.bottom = RectDrawText.top + g_LineAnchorYStep;
			if(SendMessage(ButtonSetType1, BM_GETCHECK, 0, 0) == BST_CHECKED)
			{
				DrawText(hdc, TEXT("父系窗口"), -1, &RectDrawText, DT_LEFT | DT_SINGLELINE);
			}
			else
			{
				DrawText(hdc, TEXT("拥有窗口"), -1, &RectDrawText, DT_LEFT | DT_SINGLELINE);
			}
		...
		case WM_COMMAND:
		{
			switch(LOWORD(wParam))
			{
				case 114:
				{
					...
					HWND follow = (HWND)atoi(handle);

					if(SendMessage(ButtonSetType1, BM_GETCHECK, 0, 0) == BST_CHECKED)
					{
						if(follow == NULL)
						{
							//SetWindowLongPtr(target, GWL_STYLE, GetWindowLongPtr(target, GWL_STYLE)& ~WS_CHILD);
							SetParent(target, NULL);
							//SetWindowLongPtr(target, GWL_STYLE, GetWindowLongPtr(target, GWL_STYLE) | WS_POPUP);
						}
						else if(!IsWindow(follow))
						{
							MessageBox(hwnd, L"父系窗口非法", L">_<", MB_OK | MB_ICONINFORMATION);
						}
						else
						{
							//SetWindowLongPtr(target, GWL_STYLE, GetWindowLongPtr(target, GWL_STYLE) | WS_CHILD);
							SetParent(target, follow);
						}
					}
					else
					{
						...
						{
							SetWindowLongPtr(target, GWL_STYLE, GetWindowLongPtr(target, GWL_STYLE) & ~WS_CHILD);
							SetWindowLongPtr(target, GWLP_HWNDPARENT, (LONG_PTR)follow);
							SetParent(target, NULL);
							
						}
						else if(!IsWindow(follow))
						{
							MessageBox(hwnd, L"拥有窗口非法", L">_<", MB_OK | MB_ICONINFORMATION);
						}
						else
						{
							SetWindowLongPtr(target, GWL_STYLE, GetWindowLongPtr(target, GWL_STYLE) & ~WS_CHILD);
...
LRESULT CALLBACK FrameProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(msg)
	{
		...
		case WM_TIMER:
		{
			...
			g_ColorIndex = (g_ColorIndex + 1) & (ARRAYSIZE(g_ColorTable) - 1);
			hPen = CreatePen(PS_SOLID, g_FrameWidth, RGB(g_ColorTable[g_ColorIndex].r, g_ColorTable[g_ColorIndex].g, g_ColorTable[g_ColorIndex].b));
			...
			switch(LOWORD(wParam))
			{
				...
				case ID_Del:
				{
					...
					else
					{
						...
						{
							*it = Class啥[v->ClassName].back();//把最后一项覆盖在待删除项//note!不用move，因为这里是struct，成本低
							Class啥[v->ClassName].pop_back();//弹掉多余的
						}
						if(Class啥[v->ClassName].empty())//空了的话把这个键值对也删了
						{
							Class啥.erase(v->ClassName);
						}
					}
					...
					//最后才删除数据库数据，防止指针悬空
					*v = 啥.back();//将最后一个元素移动到v指向的位置//note!用不着move因为是struct
					啥.pop_back();//删除最后一个元素
					break;
				}
			}
...
{
	//带Switch的都是开关，不带Switch对应的是待修改成的值
	...
	switch(msg)
	{
		case WM_CREATE:
		{
			...
			SetWindowSubclass(EditX, EditNegaProc, 1, 0);
			...
			SetWindowSubclass(EditY, EditNegaProc, 1, 0);
			...
			else//编辑模式
			{
				...
				if(v->EnableRule)SendMessage(ButtonEnable, BM_SETCHECK, BST_CHECKED, 0);
		...
		case WM_COMMAND:
		{
			...
			switch(LOWORD(wParam))
			{
				...
				case ID_Apply:
				{
					...
					v->EnableRule = SendMessage(ButtonEnable, BM_GETCHECK, 0, 0);
...
//窗口链窗口过程
LRESULT CALLBACK ChainProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch(msg)
	{
		case WM_CREATE:
		{
			int layer = 0;
			ChainData *cd = (ChainData *)((CREATESTRUCT *)lParam)->lpCreateParams;
			...
			//显示自己
			wchar_t buf[MAX_PATH];
			_snwprintf(buf, ARRAYSIZE(buf), L"%llu", (UINT64)Self);
			LVITEMW lvi;
			lvi.mask = LVIF_TEXT;
			lvi.iItem = 0;
			lvi.iSubItem = 0;
			...
			ListView_InsertItem(ListHwnd, &lvi);
			//1窗口标题
			GetWindowTextW(Self, buf, ARRAYSIZE(buf));
			ListView_SetItemText(ListHwnd, 0, 1, buf);
			//2窗口类名
			GetClassName(Self, buf, ARRAYSIZE(buf));
			ListView_SetItemText(ListHwnd, 0, 2, buf);
			//4PID
			DWORD pid = 0;
			GetWindowThreadProcessId(Self, &pid);
			_snwprintf(buf, ARRAYSIZE(buf), L"%lu", pid);
			ListView_SetItemText(ListHwnd, 0, 4, buf);
			//3进程名
			HANDLE hProcess = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, pid);
			...
			//6层
			_snwprintf(buf, ARRAYSIZE(buf), L"%d", layer);
			ListView_SetItemText(ListHwnd, 0, 6, buf);
			...
			if(cd->type)//Parent
			{
				...
				{
					wchar_t buf[MAX_PATH];
					_snwprintf(buf, ARRAYSIZE(buf), L"%llu", (UINT64)Parent);
					...
					lvi.pszText = buf;
					ListView_InsertItem(ListHwnd, &lvi);
					++cd->selfindex;//偏移本窗口的相对index

					//1窗口标题
					GetWindowTextW(Parent, buf, ARRAYSIZE(buf));
					ListView_SetItemText(ListHwnd, 0, 1, buf);

					//2窗口类名
					GetClassName(Parent, buf, ARRAYSIZE(buf));
					ListView_SetItemText(ListHwnd, 0, 2, buf);
				...
				//找孩子
				deque<pair<HWND, int>> q;
				auto EnumWindow = [](HWND hwnd, LPARAM lParam)->BOOL
				{
					deque<pair<HWND, int>> *q = (deque<pair<HWND, int>> *)lParam;
					q->push_back({ hwnd ,q->front().second + 1 });
					return true;
				};
				q.push_front({ NULL,0 });
				EnumChildWindows(Self, EnumWindow, (LPARAM)&q);
				q.pop_front();
				while(!q.empty())
				{
					HWND Child = q.front().first;
					int index = ListView_GetItemCount(ListHwnd);
					layer = q.front().second;

					wchar_t buf[MAX_PATH];
					_snwprintf(buf, ARRAYSIZE(buf), L"%llu", (UINT64)Child);
					...
					//1窗口标题
					GetWindowTextW(Child, buf, ARRAYSIZE(buf));
					ListView_SetItemText(ListHwnd, index, 1, buf);

					//2窗口类名
					GetClassName(Child, buf, ARRAYSIZE(buf));
					ListView_SetItemText(ListHwnd, index, 2, buf);

					//4PID
					DWORD pid = 0;
					GetWindowThreadProcessId(Child, &pid);
					_snwprintf(buf, ARRAYSIZE(buf), L"%lu", pid);
					ListView_SetItemText(ListHwnd, index, 4, buf);
					...
					ListView_SetItemText(ListHwnd, index, 3, buf);

					//5可见性
					if(IsWindowVisible(Child))
					{
						ListView_SetItemText(ListHwnd, index, 5, L"是");
					}
					else
					{
						ListView_SetItemText(ListHwnd, index, 5, L"否");
					}
					//6层
					_snwprintf(buf, ARRAYSIZE(buf), L"%d", layer);
					ListView_SetItemText(ListHwnd, index, 6, buf);
					EnumChildWindows(Child, EnumWindow, (LPARAM)&q);
					q.pop_front();
				}
			}
			else
			{
				//找主人
				HWND Owner = GetWindow(Self, GW_OWNER);
				while(Owner)
				{
					wchar_t buf[MAX_PATH];
					_snwprintf(buf, ARRAYSIZE(buf), L"%llu", (UINT64)Owner);
					...
					//1窗口标题
					GetWindowTextW(Owner, buf, ARRAYSIZE(buf));
					ListView_SetItemText(ListHwnd, 0, 1, buf);

					//2窗口类名
					GetClassName(Owner, buf, ARRAYSIZE(buf));
					ListView_SetItemText(ListHwnd, 0, 2, buf);
				...
				//找仆人
				unordered_map<HWND, vector<HWND>> Relationship;
				auto EnumWindow = [](HWND hwnd, LPARAM lParam)->BOOL
				{
					HWND owner = GetWindow(hwnd, GW_OWNER);
					if(owner)
					{
						(*((unordered_map<HWND, vector<HWND>> *)lParam))
							[owner].push_back(hwnd);
					}
					return true;
				};
				//获取所有顶级窗口列表，并记录所有owner，排除没有owner的窗口
				EnumWindows(EnumWindow, (LPARAM)&Relationship);

				deque<pair<HWND, int>> q;
				q.push_front({ Self,1 });
				while(!q.empty())
				{
					for(auto &Neko : Relationship[q.front().first])
					{
						q.push_back({ Neko,q.front().second + 1 });
						int index = ListView_GetItemCount(ListHwnd);
						layer = q.front().second;

						wchar_t buf[MAX_PATH];
						_snwprintf(buf, ARRAYSIZE(buf), L"%llu", (UINT64)Neko);
						...
						//1窗口标题
						GetWindowTextW(Neko, buf, ARRAYSIZE(buf));
						ListView_SetItemText(ListHwnd, index, 1, buf);

						//2窗口类名
						GetClassName(Neko, buf, ARRAYSIZE(buf));
						ListView_SetItemText(ListHwnd, index, 2, buf);

						//4PID
						DWORD pid = 0;
						GetWindowThreadProcessId(Neko, &pid);
						_snwprintf(buf, ARRAYSIZE(buf), L"%lu", pid);
						ListView_SetItemText(ListHwnd, index, 4, buf);
						...
						//6层
						_snwprintf(buf, ARRAYSIZE(buf), L"%d", q.front().second);
						ListView_SetItemText(ListHwnd, index, 6, buf);
					}
					q.pop_front();
				}
		...
		case WM_NOTIFY:
		{
			ChainData *cd = (ChainData *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
			LPNMHDR hdr = (LPNMHDR)lParam;

			if(hdr->hwndFrom != cd->ListHwnd)break;
			if(hdr->code == NM_CUSTOMDRAW)
			{
				LPNMLVCUSTOMDRAW c = (LPNMLVCUSTOMDRAW)lParam;

				switch(c->nmcd.dwDrawStage)
				{
					case CDDS_PREPAINT:
					...
					{
						int row = (int)c->nmcd.dwItemSpec;

						if(row < cd->selfindex)
						{
							//c->clrTextBk = RGB(255, 220, 0);   // 高亮背景
							c->clrText = RGB(0, 0, 255);         // 字体颜色
							return CDRF_DODEFAULT;
						}
						else if(row > cd->selfindex)
						{
							//c->clrTextBk = RGB(255, 220, 0);   // 高亮背景
							c->clrText = RGB(255, 0, 0);         // 字体颜色
							return CDRF_DODEFAULT;
						}
						else
						{
							//c->clrTextBk = RGB(255, 220, 0);   // 高亮背景
							c->clrText = RGB(0, 0, 0);         // 字体颜色
							return CDRF_DODEFAULT;
						}
						break;
					}
			...
			else if(hdr->code == NM_DBLCLK)
			{
				...
				{
					wchar_t buf[MAX_PATH];
					ListView_GetItemText(cd->ListHwnd, iRow, 0, buf, 256);
					SetWindowText(g_EditHandle, buf);
					HWND target = (HWND)_wtoi(buf);
					FillInformation(target);
					SendMessage(hwnd, WM_CLOSE, 0, 0);
		...
		case WM_DESTROY:
		{
			free((ChainData *)GetWindowLongPtr(hwnd, GWLP_USERDATA));
			break;
		}
	}
	return DefWindowProc(hwnd, msg, wParam, lParam);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(msg)
	{
		case WM_CREATE:
		{
			...
			//初始化颜色表
			for(int i = 0; i < ARRAYSIZE(g_ColorTable); ++i)
			{
				float hue = 360.0f * i / ARRAYSIZE(g_ColorTable);
				HSVtoRGB(hue, 1.0f, 1.0f, g_ColorTable[i]);
			}
			...
			SetWindowPos(hwnd, NULL, 0, 0, g_WindowSize.cx, g_LineAnchorY[ARRAYSIZE(g_LineAnchorY) - 1], SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOZORDER);
			...
			g_ButtonNoactivate = CreateWindow(
				...
				WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
				g_FontSize.cx * 7.5,
				g_LineAnchorY[0] + g_FontSize.cy * 0.25,
				g_FontSize.cx,
				g_FontSize.cy,
				hwnd,
			...
			SetWindowSubclass(g_EditPosX, EditNegaProc, 1, 0);
			...
			SetWindowSubclass(g_EditPosY, EditNegaProc, 1, 0);
			...
			g_StaticExename = CreateWindow(
				...
				g_FontSize.cx * 21,
				g_LineAnchorY[0],
				g_ClientSize.cx - g_FontSize.cx * 21 - g_FontSize.cx * 2,
				g_FontSize.cy * 1.4,
				hwnd,
				NULL,NULL,NULL
			);
			...
			SendMessage(g_ButtonChain, WM_SETFONT, (WPARAM)g_EditFont, 0);
			...
			CreatePointToolTip(hwnd, g_ButtonAutoRule, L"右键以启用或禁用本功能", NULL, true, NULL, NULL, NULL);
		...
		case WM_TIMER://16ms
		{
			switch(wParam)
			{
				...
				{
					if(wParam == ID_Timer_LSelect && !(GetAsyncKeyState(VK_LBUTTON) & 0x8000))//是鼠标计时器但是鼠标松开了
					{
						//关掉这个计时器并删除框框
				...
				{
					if(wParam == ID_Timer_RSelect && !(GetAsyncKeyState(VK_RBUTTON) & 0x8000))//是鼠标计时器但是鼠标松开了
					{
						//关掉这个计时器并删除框框
		...
		case WM_COMMAND:
		{
			switch(LOWORD(wParam))
			{
				...
				case ID_MENU_TOP:
				{
					...
					RECT rc;
					GetWindowRect(target, &rc);
					if(g_ShareTopTooltip)DestroyWindow(g_ShareTopTooltip); g_ShareTopTooltip = NULL;
					ShowTooltip(target, &g_ShareTopTooltip, { rc.left,rc.top }, TEXT("已置顶"), NULL, true, NULL, TTI_NONE, 1000);
					break;
				}
				case ID_MENU_NOTOP:
				{
					...
					GetWindowRect(target, &rc);
					if(g_ShareTopTooltip)DestroyWindow(g_ShareTopTooltip); g_ShareTopTooltip = NULL;
					ShowTooltip(target, &g_ShareTopTooltip, { rc.left,rc.top }, TEXT("取消置顶"), NULL, true, NULL, TTI_NONE, 1000);
					break;
				}
				...
				case ID_MENU_ChainParent:
				{
					wchar_t handle[HANDLESIZE];
					GetWindowText(g_EditHandle, handle, ARRAYSIZE(handle));
					HWND target = (HWND)_wtoi(handle);
					if(!IsWindow(target))
					{
						MessageBox(hwnd, L"目标窗口非法", L">_<", MB_OK | MB_ICONINFORMATION);
						break;
					...
					g_ChainHWND = CreateWindowEx(
						(g_Noactivate == BST_CHECKED ? WS_EX_NOACTIVATE : NULL) | WS_EX_TOPMOST,
						L"Window's Handle - Chain",
						(wstring(handle) + wstring(L" - 父级窗口链")).c_str(),
						WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_POPUP | WS_SYSMENU,
						rect.left,
						rect.top,
						rect.right - rect.left,
				...
				case ID_MENU_ChainOwner:
				{
					...
					g_ChainHWND = CreateWindowEx(
						(g_Noactivate == BST_CHECKED ? WS_EX_NOACTIVATE : NULL) | WS_EX_TOPMOST,
						L"Window's Handle - Chain",
						(wstring(handle) + wstring(L" - 拥有窗口链")).c_str(),
						WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_POPUP | WS_SYSMENU,
						rect.left,
						rect.top,
						rect.right - rect.left,
				...
				case ID_BTN_Style:
				{
					...
					RECT rect;
					GetWindowRect(hwnd, &rect);
					rect.left = ((rect.right + rect.left) >> 1) - 16 * g_FontSize.cx;
					rect.top = ((rect.bottom + rect.top) >> 1) - 18.5 * g_FontSize.cy;
					rect.right = rect.left + 32 * g_FontSize.cx;
				...
				case ID_BTN_SetFollow:
				{
					...
					if(IsWindow(g_SetFollowHWND))
					{
						ShowWindow(g_SetFollowHWND, SW_NORMAL);
						SetForegroundWindow(g_SetFollowHWND);
						SetWindowPos(g_SetFollowHWND, NULL, rect.left, rect.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
						break;
					}
					rect.right = rect.left + 16 * g_FontSize.cx;
					rect.bottom = rect.top + g_LineAnchorY[4];
					AdjustWindowRectEx(&rect, WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_POPUP | WS_SYSMENU, false, (g_Noactivate == BST_CHECKED ? WS_EX_NOACTIVATE : NULL));
					if(rect.top < 0)//屏幕上外
					{
						rect.bottom -= rect.top;
					...
					g_SetFollowHWND = CreateWindowEx(
						(g_Noactivate == BST_CHECKED ? WS_EX_NOACTIVATE : NULL),
						L"Window's Handle - SetFollow",
						L"窗口从属",
						WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_POPUP | WS_SYSMENU,
						rect.left,
						rect.top,
				...
				case ID_BTN_Chain:
				{
					...
					// 根据 status 添加第一个项
					AppendMenu(hPopupMenu, MF_STRING, ID_MENU_ChainParent, L"父级窗口链");
					AppendMenu(hPopupMenu, MF_STRING, ID_MENU_ChainOwner, L"拥有窗口链");
...
ABOUT:
		...
		case WM_PAINT:
		{
			...
			RectDrawText.top = g_LineAnchorY[5];
			RectDrawText.bottom = RectDrawText.top + g_LineAnchorYStep;
			DrawText(hdc, TEXT("普通样式"), -1, &RectDrawText, DT_RIGHT | DT_SINGLELINE);

			RectDrawText.top = g_LineAnchorY[6];
			RectDrawText.bottom = RectDrawText.top + g_LineAnchorYStep;
			DrawText(hdc, TEXT("扩展样式"), -1, &RectDrawText, DT_RIGHT | DT_SINGLELINE);
		...
		case WM_Tray:
		{
			if(lParam == WM_RBUTTONUP)
			{
				...
				AppendMenu(hPopupMenu, MF_STRING, 2, L"关于");
				AppendMenu(hPopupMenu, MF_STRING, 3, L"退出");
				POINT pt;
				GetCursorPos(&pt);
				SetForegroundWindow(hwnd);
				// 显示菜单
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevIntance, LPSTR szCmdline, int iCmdShow)
{
	...
	wndclassex.lpfnWndProc = SetFollowProc;//窗口过程(回调函数)
	wndclassex.lpszClassName = TEXT("Window's Handle - SetFollow");//窗口类名
	if(!RegisterClassEx(&wndclassex))//注册并检验窗口
	{
		MessageBox(NULL, TEXT("这个程序需要在Windows NT才能执行！"), g_sTitle, MB_ICONERROR);
		return 0;
	...
	wndclassex.lpfnWndProc = ChainProc;//窗口过程(回调函数)
	wndclassex.lpszClassName = TEXT("Window's Handle - Chain");//窗口类名
	if(!RegisterClassEx(&wndclassex))//注册并检验窗口
	{
		MessageBox(NULL, TEXT("这个程序需要在Windows NT才能执行！"), g_sTitle, MB_ICONERROR);
		return 0;
	...
	g_MainHwnd = CreateWindowEx(
		...
		(GetSystemMetrics(SM_CXSCREEN) - g_WindowSize.cx) >> 1,
		(GetSystemMetrics(SM_CYSCREEN) - g_WindowSize.cy) >> 1,
		g_WindowSize.cx,
		g_WindowSize.cy,
		NULL,//父窗口句柄
		NULL,//菜单句柄
